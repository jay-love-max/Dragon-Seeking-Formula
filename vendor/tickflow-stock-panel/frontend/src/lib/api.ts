...
export interface MarketRecap {
  ...
  f18_numerator: number | null
  f18_denominator: number | null
  f18_policy: string | null
  f18_risk_budget: number | null
  f18_low_sample: boolean | null
}

export interface ExecutionPlan {
  action: 'CONDITIONAL_BUY' | 'EXIT' | 'HOLD' | 'WATCH' | 'REDUCE' | 'NO_TRADE'
...
export interface Candidate {
  ...
  block_f16: number | null
  block_f17: number | null
  block_f18: number | null
  block_f19: number | null
  buy_plan: ExecutionPlan | null
  defensive_plans: ExecutionPlan[]
}
...
export interface UziAuditRecord {
  date: string
  code: string
  name: string
...
export interface RecapDataResponse {
  history: RecapHistoryItem[]
  calibration: CalibrationBucket[]
  uzi_audit: UziAuditRecord[]
}
...
export const api = {
  ...
  clearTickflowKey: () =>
    request<unknown>('/api/settings/tickflow-key', { method: 'DELETE' }),
