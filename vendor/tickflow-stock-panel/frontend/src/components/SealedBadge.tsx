...
function SealedDirBlock({ title, color, counts, rawTotal }: {
  ...
  const accentColor = isBull ? 'var(--bull, #F04438)' : 'var(--bear, #12B76A)'
  return (
    <div className="mb-2 last:mb-0">
      <div className="flex items-center justify-between px-1 py-0.5 rounded" style={{ background: `color-mix(in srgb, ${accentColor} 8%, transparent)` }}>
        <span className="text-[10px] font-medium" style={{ color: accentColor }}>{title}</span>
        <span className="tabular-nums text-[10px]">
          <span className="text-muted line-through">{original}</span>
          <span className="text-muted/50 mx-1">→</span>
          <span className="font-bold" style={{ color: accentColor }}>{fixed}</span>
        </span>
      </div>
      <div className="flex gap-3 px-1 text-[10px]">
        <span className="flex items-center gap-0.5" style={{ color: accentColor }}>
          <span className="h-1 w-1 rounded-full" style={{ background: accentColor }} />真封 {real}
        </span>
        <span className="flex items-center gap-0.5 text-yellow-500">
          <span className="h-1 w-1 rounded-full bg-yellow-500" />假 {fake}
        </span>
        {pending > 0 && (
          <span className="flex items-center gap-0.5 text-muted">
            <span className="h-1 w-1 rounded-full bg-muted" />待 {pending}
          </span>
        )}
      </div>
    </div>
