...
const SOURCE_BADGE: Record<string, { label: string; cls: string }> = {
  strategy:  { label: '策略',   cls: 'bg-warning/15 text-warning' },
  signal:    { label: '信号',   cls: 'bg-accent/15 text-accent' },
  price:     { label: '价格',   cls: 'bg-bear/15 text-bear' },
  market:    { label: '异动',   cls: 'bg-accent/15 text-accent' },
  new_entry: { label: '进入',   cls: 'bg-bear/15 text-bear' },
...
export function AlertToastContainer() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[9999] flex flex-col gap-2 w-[320px] pointer-events-none" role="alert" aria-live="polite">
      <AnimatePresence>
        {items
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={(e) => { e.stopPropagation(); dismiss(item.id) }} aria-label="关闭通知" className="shrink-0 p-0.5 rounded text-muted/50 hover:text-foreground hover:bg-elevated transition-colors cursor-pointer">
                  <X className="h-3 w-3" />
                </button>
              </div>
              ...
              {isStrategy ? (
                <div className="mt-1 flex items-center gap-1.5 pl-0.5">
                  ...
                  <span className="text-[11px] font-medium text-warning">「{sname}」</span>
