    ...
    buildSeries: (data) => {
      ...
      return [
        ...
        },
        {
          ...
          lineStyle: { width: 1, color: CHART.vol5 },
          itemStyle: { color: CHART.vol5 },
        },
        {
          name: 'VOL10',
          type: 'line',
          data: ma10Data,
          smooth: true, symbol: 'none', animation: false,
          lineStyle: { width: 1, color: 'CHART.vol10' },
          itemStyle: { color: 'CHART.vol10' },
        },
      ]
    },
    buildInfo: (d) => {
      if (!d) return []
      return [
        { label: '量', color: d.close >= d.open ? 'CHART.bull' : 'CHART.bear', value: fmtVol(d.volume) },
      ]
    },
  },
  {
    ...
    buildInfo: (d) => {
      if (!d) return []
      return [
        { label: 'DIF', color: CHART.vol5, value: d.macd_dif != null ? d.macd_dif.toFixed(3) : '—' },
        { label: 'DEA', color: 'CHART.vol10', value: d.macd_dea != null ? d.macd_dea.toFixed(3) : '—' },
        { label: 'MACD', color: d.macd_hist != null && d.macd_hist >= 0 ? 'CHART.bull' : 'CHART.bear', value: d.macd_hist != null ? d.macd_hist.toFixed(3) : '—' },
      ]
    },
  },
  {
    ...
    buildSeries: (data) => [
      ...
      },
      {
        ...
        type: 'line',
          ...
          smooth: true, symbol: 'none', animation: false,
          lineStyle: { width: 1, color: CHART.accent },
    ...
    buildInfo: (d) => {
      if (!d) return []
      return [
        { label: 'RSI6', color: CHART.vol5, value: d.rsi_6 != null ? d.rsi_6.toFixed(1) : '—' },
        { label: 'RSI14', color: CHART.accent, value: d.rsi_14 != null ? d.rsi_14.toFixed(1) : '—' },
        { label: 'RSI24', color: 'CHART.vol10', value: d.rsi_24 != null ? d.rsi_24.toFixed(1) : '—' },
      ]
    },
  },
  {
    ...
    buildInfo: (d) => {
      if (!d) return []
      return [
        { label: 'K', color: CHART.vol5, value: d.kdj_k != null ? d.kdj_k.toFixed(1) : '—' },
        { label: 'D', color: CHART.accent, value: d.kdj_d != null ? d.kdj_d.toFixed(1) : '—' },
        { label: 'J', color: 'CHART.vol10', value: d.kdj_j != null ? d.kdj_j.toFixed(1) : '—' },
      ]
    },
  },
]
...
const THEME = {
  bull: CHART.bull,
  bear: CHART.bear,
  bullAlpha: CHART.bullAlpha,
  bearAlpha: CHART.bearAlpha,
  ma5: CHART.ma5,
  ma10: CHART.ma10,
  ma20: CHART.ma20,
  ma60: CHART.ma60,
...
function buildSubInfoGraphics(
  ...
  activeIndicators.forEach((key) => {
    ...
    if (def.key === 'vol' && d) {
      ...
      items.push({ label: 'VOL5', color: CHART.vol5, value: fmtVol(vol5) })
      items.push({ label: 'VOL10', color: 'CHART.vol10', value: fmtVol(vol10) })
    }
    ...
    graphics.push({
      ...
      style: { stroke: 'CHART.sep', lineWidth: 1 },
      silent: true, z: 0,
    })
    graphics.push({
      id: `sub-label-${key}`,
