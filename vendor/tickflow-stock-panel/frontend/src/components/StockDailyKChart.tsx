...
function buildLimitUpMarkers(rows: KlineRow[]): ChartMarker[] {
  ...
  for (const r of rows) {
    ...
    if (r.signal_broken_limit_up) {
      markers.push({ date, kind: 'neutral', above: true, color: CHART.purple, label: '炸' })
    } else if (r.signal_limit_up) {
      const boards: number = r.consecutive_limit_ups ?? 1
      markers.push({ date, kind: 'buy', above: true, color: CHART.yellow, label: boards <= 1 ? '板' : String(boards) })
    }
  }
  return markers
}
...
StockDailyKChart.displayName = 'StockDailyKChart'
