...
def run_now(
    ...
    import shutil
    import subprocess
    project_root = settings.data_dir.parent.parent
    recap_script = project_root / "src" / "recap_engine.py"
    if recap_script.exists():
        emit("recap_engine", 98, "运行寻龙诀复盘引擎计算…")
        try:
            python_bin = shutil.which("python3") or shutil.which("python") or sys.executable
            logger.info("Running daily recap engine using %s: %s", python_bin, recap_script)
            res = subprocess.run(
                [python_bin, str(recap_script)],
                cwd=str(project_root),
                capture_output=True,
                text=True,
                check=True
            )
            logger.info("Recap engine stdout: %s", res.stdout)
            emit("recap_engine", 99, "寻龙诀复盘引擎计算完成")
        except Exception as e:
            logger.error("Failed to run daily recap engine: %s", e)
            emit("recap_engine", 99, f"寻龙诀复盘计算失败: {e}")
    else:
        logger.warning("Recap engine script not found at %s", recap_script)
